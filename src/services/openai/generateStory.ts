...
export async function generateStory(props: GenerateStoryProps) {
    ...
    try {
        const promptForOpenAI = generateOpenAIStoryPrompt(props.prompt, props.language, props.languageLevel as languageLevel,"medium")
        ...
        if (!parsed.english_version || !parsed.translated_version) {
            ...
            throw new Error("AI response missing required story versions..please try again");
        }
